				...
				<div role="tabpanel" class="tab-pane <?=($reload == 'orders')?'active':'';?>" id="orders">
					...
					<table style="width:100%">
						...
						<tbody>
							...
							// function arraysortHandling($a, $b){
							// 	if($a['STATUS_ID'] == 'N'){
							// 		return -1;
							// 	}
							// 	if($b['STATUS_ID'] == 'N'){
							...
							<tr class="<?=$row['Xtype']?>">
								<td data-city="<?=$row['CITYFROM']?>" data-office="<?=$row['OFFICE']?>" class="fulltr" colspan="9">
									<?if ($row['Xtype']=='podbor'){?>Заявки на подбор тура<?}?>
									<?if ($row['Xtype']=='sale'){?>Заявка  c онлайн оплатой<?}?>
								...
								<tr data-city="<?=$row['CITYFROM']?>" data-office="<?=$row['OFFICE']?>" class="showmeall <?=$row['Xtype']?>" data-orderid="<?=$row['ID']?>">
									...
									<td><?if (!empty($row['RESPONSIBLE_ID'])) {
										...
										<td>
											...
											<a class="edit-comment order link <?=$row['Xtype']?>"><img src="<?=SITE_TEMPLATE_PATH?>/images/paladdin/edit.png" alt=""></a>
											<?}?>
										</td>
										<td><select name="statusOrder" class="changeStatus <?=$row['Xtype']?>">
											...
											<option value="N" <?if($row['STATUS_ID']=="N") echo "selected";?>><i class="fa neworder"></i> Новая заявка</option>

											<? if($row['Xtype']=='sale' || $row['Xtype']=='podbor') {?>
											<option value="DN" <?if($row['STATUS_ID']=="DN") echo "selected";?>><i class="fa fa-repeat"></i> В обработке</option>
											<option value="A" <?if($row['STATUS_ID']=="A") echo "selected";?>>Авторизация</option>
											<option value="ZT" <?if($row['STATUS_ID']=="ZT") echo "selected";?>>Запрос к ТО</option>
											<option value="PT" <?if($row['STATUS_ID']=="PT") echo "selected";?>>Подтвержден ТО</option>
											<option value="P" <?if($row['STATUS_ID']=="P") echo "selected";?>>Оплачен</option>

											<? }?>

											<option value="F" <?if($row['STATUS_ID']=="F") echo "selected";?>>Обработано</option>
										...
										<td colspan=8>
											<div class="fulltable">
												<div style="padding:20px;">
													<div class="row">
														<?if ($row['Xtype']=='sale'){
															...
															<div class="col-md-6">
																...
																<div class="answered-question-answer" data-id="<?=$row['ID']?>" style="width: 400px">
																	<?=$row['ANSWER']?>
																</div>
																<? }
																else {?>
																...
																<?
																if(!$answer_already_exists){?>
																	<label class="checkbox"><input type="checkbox">&nbsp;Опубликовать Ответ на сайте </label>
	
																	<button data-id="<?=$row['ID']?>" class="publication">
																		ОТПРАВИТЬ ОТВЕТ
																	</button>
																	<?if(0):?>
																		<button data-id="<?=$row['ID']?>" class="tochatthisshit">
																			ПЕРЕЙТИ К ЧАТУ
																		</button>
																	<?endif?>	
																<? }?>
													...
													<div class="bottominf">
														<span><img src="/bitrix/templates/tour/images/paladdin/close.png" alt=""></span>
													</div>
												</div>
											</td>
										</tr>
							...
							<div role="tabpanel" class="tab-pane <?if(isset($rqID)){?>active<?}?>" id="vitturs">
								...
								<p>Вы вошли как <span><i class="fa fa-user"></i> <?=$USER->GetFullName()?></span> - <?
									...
									<h3>Горящие туры</h3>
									<div class="lk-menu">
										<button class="action first <?if(!isset($rqID)){?>active<?}?>" data-id="list" >Список</button>
										<button class="action last" data-id="addedit" ><?if(isset($rqID)){?>Редактировать<?}else{?>Добавить<?}?></button>
...
<div class="row">
	<div class="col-md-4">
		<div class="form-group ">
			...
			<input  style="display:none;" type="file" id="file-tour-cover" name="cover" class="form-control" />
		...
		<div class="form-group ">
			...
			<select name="cityz" class="form-control" <?if(isset($rqID)){?>data-editid="<?=$arProps[0]['DEPARTURE']['VALUE']?>"<?}?>>
				
			</select>
			<input type="hidden" name="tvid" value="<?if(isset($rqID)){?><?=$turs['UF_DEPARTURE']?><?}else{?>0<?}?>">
		</div>
		<div class="form-group ">
			<label for=" ">Страна</label>
			<select name="countryz" class="form-control" <?if(isset($rqID)){?>data-editid="<?=$arProps[0]['COUNTRY']['VALUE']?>"<?}?>>
...
<div class="row">
	<div class="col-md-4">	
		<div class="form-group ">
			...
			<select name="curort" class="form-control" <?if(isset($rqID)){?>data-editid="<?=$arProps[0]['CURORT']['VALUE']?>"<?}?>>
	...
	<div class="col-md-4 hiddenifneed">	
		<div class="form-group ">
			...
			<input id=" " name="curort_new" value="<?if(isset($rqID)){?><?=$arProps[0]['CURORT']['VALUE']?><?}?>"  class="form-control" />
		</div>
	</div>
</div>
	...
	<div class="col-md-12 appendtome">
		...
		foreach ($arFieldz as $key=> $row) {
			...
			<div class="hotBaseAndDate" data-rid="<?if(isset($row['ID']))echo $row['ID']; else echo '1';?>">
				<div class="whiteds">
					<h4>Дата вылета #<b><?if(isset($row['ID']))echo $row['ID']; else echo '1';?></b> <span style="float:right;display:inline-block;"><label><input type="checkbox" name="dates[<?=$z;?>][turhide]" class="copyme" <?if($row['ACTIVE']=='N'){?>checked<?}?>> Скрыть</label></span></h4>
					<div class="row">
						<div class="col-md-2">
							<div class="form-group ">
								<label for=" ">Туроператор </label>
								<select name="dates[<?=$z;?>][turoper]" class="form-control turopers copyme" <?if(isset($rqID)){?>data-editid="<?=$arProps[$key]['TUROPERATOR']['VALUE']?>"<?}?>>
								</select>
							</div>
						</div>
						<div class="col-md-3">
...
<div class="row">
	<div class="col-md-12">
		...
		<?
		if(isset($row2['ID'])) {
			?><input type="hidden" name="dates[<?=$z;?>][hotel_base][<?=$k-1;?>][hotel_id]" value="<?=$row2['ID']?>" />
			...
			<div class="row">
				...
				<div class="col-md-3">
					<div class="form-group ">
						...
						<input name="dates[<?=$z;?>][hotel_base][<?=$k-1;?>][name]" class="form-control hotelzname copyme" <?if(!empty($row2['NAME'])){?>value="<?=$row2['NAME']?>" <?}?>/>
				...
				<div class="col-md-2">
					<div class="form-group ">
						<select name="dates[<?=$z;?>][hotel_base][<?=$k-1;?>][stars]"  class="form-control copyme">
							...
							<option value="5" <?if($arProps3[$key1]['hotelstars']['VALUE']==5||empty($arProps3[$key1]['hotelstars']['VALUE'])){?>selected<?}?>>★★★★★</option>
				...
				<div class="col-md-2">
					<div class="form-group ">
						...
						<input id=" " name="dates[<?=$z;?>][hotel_base][<?=$k-1;?>][price]" class="form-control copyme" <?if(!empty($arProps3[$key1]['price']['VALUE'])){?>value="<?=$arProps3[$key1]['price']['VALUE']?>" <?}?>/>
					</div>
				</div>
				<div class="col-md-1">
					<div class="form-group ">
	...
	<div class="row">
		<div class="col-md-6">
									...
									<div class="row">
										<div class="col-md-12">
											<div class="form-group ">
												...
												<textarea name="dates[<?=$z?>][document]" class="form-control notepad copyme" rows=6><?=$arProps[$key]['DOCUMS']['VALUE']?></textarea>
											</div>
										</div>
									</div>
			...
			<button type="submit" class="activebtn"><?if(isset($rqID)){?>внести изменения<?}else{?>Добавить<?}?></button>
				...
				<div class="col-md-6">
					<div class="chatlist">
						<div class="chatmenu">
							...
							<span class="refresh-list"><i class="fa fa-refresh " aria-hidden="true"></i></span>
						</div>
						<div class="msg ">
							<?
							if(!CModule::IncludeModule("form")) die();
...
if(in_array(1,$USER->GetUserGroupArray())|| in_array(10,$USER->GetUserGroupArray())  || $USER->GetEmail() == $arrAnswersVarname[$arResult['ID']]["EMAIL"][0]["USER_TEXT"]){
	...
	?>										
	<div class="msglines <?=$unreadClass?>" id="<?=strtotime($lastDate)?>" data-id="<?=$arResult['ID']?>" <? if($dead_detected) {?> thereisdead="true"<? }?>>
	<!-- <div class="pict">
		<div class="img"><img src="/bitrix/templates/tour/images/user_03.jpg" /></div>
	</div> -->
	<div class="infomsg">
		<h4><?=$arrAnswersVarname[$arResult['ID']]["NAME"][0]["USER_TEXT"]?> (Вопрос #<?=$arResult['ID']?>)</h4>
	...
	<div class="onlinechat" >
		...
		<div class="chatbot">
			<form id="chatmeplease">
				...
				<div class="loadingspinner">Загрузка <i class="fa fa-spinner fa-pulse fa-3x fa-fw" aria-hidden="true"></i></div>
			</form>
		</div>
	</div>
</div>
