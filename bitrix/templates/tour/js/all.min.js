...
function renderEmloeyrs(city, tvcity){
	...
	$.ajax({
		...
		success: function(data){
			...
			item_count = $('.workers .item').length;
			if(item_count >= 3) items = 3
			else items = item_count;
			console.log('item = '+item_count);
			if($.trim($('.addme').html()) == '') {
				$('.magic').remove()
				if(item_count >= 4) items = 4
				else items = item_count;
	...
	$.ajax({
		...
		success: function(e) {
			...
			if(e.search('input') ==  -1){
				...
				if(t == 7) {
					var html = '<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 class="modal-title" id="myModalLabel"></h4></div><div class="thnx"><h1>Спасибо!</h1><p>Ваша оценка принята</p></div>'
					e = html
				}
...
function getCityByIp(t, callback) {
	...
	$.ajax({
		...
		success: function(t) {			
			...
			console.log(t);
			if(t.length > 0){
				$('#mapcontainer').removeClass('zero-height');
				$('#mapcontainer').css({'height':$('#mapcontainer').css('max-height')});
				$('.mapover').show();
			}
			else{
				$('#mapcontainer').addClass('zero-height');
				$('.mapover').hide();
			}
...
$("video").length > 0 && plyr.setup({
	...
	t.preventDefault(),
	id = $(this).data("city"),
	txt = $(this).attr('data-citytvname'),
	$(".adressAll").hide(),
	$(".fz-city :contains('" + txt + "'), .city_departure :contains('" + txt + "')").attr("selected", "selected"),
	...
	load_tours(e, city_id, true, city_name)
	...
	$.ajax({
	    ...
	    success: function(data){
			...
			$('select').styler('destroy');
			$('#select-office option').remove();
			for(var i in data){				
				$('select#select-office').append('<option data-id="'+data[i]+'" value="'+i+'">'+data[i]+'</option>');
			}			
			initDropdown();
		}
	})
...
$(document).on("click", ".cityAll2 .letterlist a", function(t) {
	...
	var tx = $(this).attr('data-citytvname')
...
$(document).on("submit", "#jobform form", function(t) {
	...
	request.open('POST', '/ajax/form.php?id=6', true);

	var fd = new FormData(),
		name = $('input[name = form_text_46]').val(),
		phone = $('input[name = form_text_47]').val(),
		skype = $('input[name = form_text_48]').val(),
		email = $('input[name = form_email_49]').val(),
		msg = $('#jobform form textarea').val(),
		cv = $('input[name = form_file_113]')[0].files[0],
		foto = $('input[name = form_file_51]')[0].files[0];
	fd.append("form_text_46", name);
	fd.append("form_text_47", phone);
	fd.append("form_text_48", skype);
	fd.append("form_email_49", email);
	fd.append("form_textarea_50", msg);
	fd.append("form_file_113", cv);
	fd.append("form_file_51", foto);
				...
				$(".dtp2").datetimepicker({
					...
					$(document).on("click", ".bigbtn.ank", function(t) {
						...
						$.ajax({
							...
							processData: !1,
							success: function(t) {
								"ok" == t ? ($("#addcompany").modal("hide"), $("#anketaComp")[0].reset(), $("#answer .modal-content").html('<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button> <h4 class="modal-title" id="myModalLabel"></h4></div><div class="thnx"><h1>Спасибо, ваша заявка принята!</h1><p>Ваша заявка будет отражена в разделе "попутчкик" после провреки моедратором сайта</p></div>')) : $("#answer .modal-content").html(t), Recaptchafree.reset(), $("#answer").modal("show")
							}
						})
					}), 
