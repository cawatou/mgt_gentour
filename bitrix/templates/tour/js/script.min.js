$(document).ready(function () {
    ...
    var city_departure = $('.letterlist .active').attr('data-citytvname');
    console.log(city_departure);
    $('.city_departure').text(city_departure);

    // Загружаем слайдер на странице "Все сотрудники"
    var location = window.location.href;
    if(location.search('/content/employe/') != -1) renderEmloeyrs('', '');
    if(location.search('/content/teach/') != -1) {
        var hash = window.location.hash;
        setTimeout(function(){
            hash = "#pro" + hash.slice(1);
...
$('form[name=franch]').on('submit', function(e){
    e.preventDefault();
    console.log(this);
    franchise_submit();
})
$('form.franchise').on('submit', function(e){
    e.preventDefault();
    console.log(this);
    franchise_submit();
})

$('#question_franchise form').on('submit', function(e){
    e.preventDefault();
    var data = $(this).serialize();
    console.log(data);
    $.ajax({
...
$('form[name=teahme]').on('submit', function(e){
    e.preventDefault();
    var data = $(this).serialize();
    console.log(data);
    $.ajax({
...
$('.open_course').on('click', function(){
    ...
    $('select').styler('destroy');
    $("#progsselect").val(title);
    $('input[name=form_text_62]').val(title);
    initDropdown();
})
...
function load_tours(city_id, city_idSecond, reload, city_name) {
    ...
    console.log("before reload -"+city_name);
    if (reload) {
        console.log("after reload -"+city_name);
...
function franchise_submit(){    
    console.log("franchise_submit");
    var name,
        phone;
    if($('input[name=form_text_84]').val()) name = $('input[name=form_text_84]').val();
    if($('.franchise input[name=name]').val()) name = $('.franchise input[name=name]').val();
    if($('input[name=form_text_85]').val()) phone = $('input[name=form_text_85]').val();
    if($('.franchise input[name=phone]').val()) phone = $('input[name=phone]').val();
    ...
    $.ajax({
        ...
        success: function(res){
            $('.franchise input[name=name]').val('');
            $('.franchise input[name=phone]').val('');
